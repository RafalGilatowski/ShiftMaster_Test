/*
  # VMS Full Production Schema

  ## Tables
  - `profiles` - user profiles linked to auth.users (id, role, full_name, company_name, email)
  - `orders` - workforce orders created by internal users
  - `offers` - supplier offers submitted against orders

  ## Security
  - RLS enabled on all tables
  - Separate policies per operation (SELECT/INSERT/UPDATE)
  - Role-based access via JWT user_metadata

  ## Triggers
  - `on_auth_user_created` → auto-inserts profile row on signup
  - `offers_updated_at` → auto-updates updated_at on offer changes
*/
...
-- Trigger: update updated_at on offers
